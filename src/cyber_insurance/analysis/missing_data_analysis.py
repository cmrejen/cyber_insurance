...
class MissingDataAnalyzer:
    ...
    def prepare_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col, values in unknown_mappings.items():
            ...
            self._missing_indicators[col] = pd.Series(
                self._df[col].isnull().astype(int), name=f"{col}_missing"
            )
            ...
            logger.info(
                f"Missing values in {col}: {missing_count} " f"({missing_pct:.2f}%)"
            )

        return self._df

    def analyze_mar_patterns(
        self, save_path: Optional[Path] = None, use_encoded: bool = False
    ) -> None:
    ...
    def _analyze_encoded_patterns(
        self, target_col: str, indicator: pd.Series, save_path: Optional[Path]
    ) -> None:
        ...
        correlations = pd.Series(
            {
                col: stats.pointbiserialr(self._df_encoded[col], indicator).correlation
        ...
        if save_path:
            plot_path = (
                OutputPaths.MISSING_CONTINUOUS
                / f"{target_col}_encoded_correlations.png"
            )
    ...
    def _analyze_temporal_patterns(
        ...
        if save_path:
            plot_path = (
                OutputPaths.MISSING_TEMPORAL / f"{target_col}_temporal_pattern.png"
            )
            plt.savefig(plot_path)
    ...
    def _analyze_categorical_patterns(
        ...
        contingency = pd.crosstab(self._df[category_col], indicator)
        ...
        missing_by_cat = (contingency[1] / contingency.sum(axis=1) * 100).sort_values(
            ascending=False
        )
        ...
        if save_path:
            plot_path = (
                OutputPaths.MISSING_CATEGORICAL
                / f"{target_col}_{category_col}_pattern.png"
            )
            plt.savefig(plot_path)
    ...
    def plot_missing_heatmap(self, save_path: Optional[Path] = None) -> None:
        ...
        sns.heatmap(self._df.isnull(), cbar=False, cmap="crest", yticklabels=False)
    ...
    def plot_missing_by_year(self, save_path: Optional[Path] = None) -> None:
        ...
        for col, values in missing_by_year.items():
            plt.plot(values.index, values.values, marker="o", label=col)
...
def main() -> None:
    ...
    analyzer.plot_missing_heatmap(output_dir / "missing_heatmap.png")

    analyzer.plot_missing_by_year(output_dir / "missing_by_year.png")
